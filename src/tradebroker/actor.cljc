(ns tradebroker.actor
  "TradeBrokerageActor — the ISCO-08 3324 independent trade
  brokerage actor as a `langgraph.graph/state-graph` (ADR-2607011000 /
  CLAUDE.md Actors section). One graph run = one deal confirmation or
  shipment filing operation request (intake → advise → govern → decide →
  ...
  can resume after human sign-off. Modeled on cloud-itonami-isco-3322's
  salesrep.actor.
  ...
  The unconditional invariant: the Trade Advisor can never
  directly confirm a deal or file a shipment the
  TradeBrokerageGovernor refuses — every commit-record! call is
  gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [tradebroker.advisor :as advisor]
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:principal-id (:principal-id request)
                                    :op (:op proposal)
                                    :deal-id (:deal-id proposal)
                                    :payload proposal}]
